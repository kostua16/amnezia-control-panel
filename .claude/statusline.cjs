...
} = require('./hooks/lib/statusline-session-cache.cjs');
const {
  readUsageCache,
...
async function readStdin() {
  return new Promise((resolve, reject) => {
    ...
    const parsedTimeout = Number.parseInt(
      env.CK_STATUSLINE_STDIN_TIMEOUT_MS || '',
      10,
    );
    const timeoutMs =
  ...
  try {
    ...
    try {
      ...
      if (sessionId) {
        ...
        transcript =
          readActivitySnapshot(sessionId, readSessionState) || transcript;
      }
    } catch {}
      ...
      try {
        ...
        fs.writeFileSync(
          ...
          JSON.stringify({
            ...
            remaining:
              data.context_window?.remaining_percentage ?? 100 - contextPercent,
            tokens: totalTokens,
            size: contextSize,
            usage,
            timestamp: Date.now(),
    ...
    const statuslineMode =
      config.statuslineLayout?.baseMode || config.statusline || 'full';
    const usageWindows =
