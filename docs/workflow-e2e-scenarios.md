...
| M11b | `workflow_run` completed but `gh pr checks` still reports required jobs pending | completed workflow-run jobs reconcile stale pending rows; skipped CI jobs stay non-blocking → M13 | spec                                            |
