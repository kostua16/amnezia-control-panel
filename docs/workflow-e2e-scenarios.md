...
| P11h | spun-off sub-issue (`umbrella-sub-issue` + `auto-fix` + priority label, triaged on open)                                                                                                                           | flows through the NORMAL pipeline: triage → catch-up Phase 6 auto-`/fix` (automation-authored, low/medium/high) → fix PR `Closes #sub` → sub-issue closes → P11g ticks the umbrella; the `umbrella-sub-issue` label makes `isTrackingIssue` return false (despite the "umbrella tracking issue" body phrase), so triage/catch-up/PM treat it as a normal fixable issue                                                                                                                  | char          |
...
Decision basis: `fix-review.yml` issue_comment (`/fix-review`,`/address-review`) + `workflow_dispatch`; concurrency "ignored" branch; ZAI address-review → commit. Distinct command-driven fix flow (not folded into §3). Push is gated by `validate-pr-gate` (`id: gate`); a `detect-noop` step skips the gate+push and posts `renderNoChanges` when the agent changed nothing. `detect-noop` counts only pushable changes: `.claude-pr/` (runtime artifact) and `.github/actions/` (force-restored before commit) are excluded, so agent edits confined to them read as a no-op and reach the false-positive code-review retrigger instead of ending in limbo (has_changes=true, nothing pushed, no retrigger). A gate failure posts a dual-block summary (agent-reported vs authoritative gate) and does not push.
