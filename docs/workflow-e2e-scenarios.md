...
| X2   | **`GH_PAT` expired / insufficient scope**                                                            | `upsert-pull-request`/`commit-and-push` fail → report-failure across agents; workflow-file rejects are reported as missing `workflow` / `Workflows: write` grant | reported (no silent corruption)                   | char                        |
