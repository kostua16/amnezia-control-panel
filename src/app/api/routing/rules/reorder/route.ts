...
export async function POST(request: NextRequest) {
  try {
    ...
    // Batch update priorities using a transaction
    ...
    await writeAuditLog({
      action: 'routing.rule.reorder',
      resource: 'routingRule',
      metadata: { updated: rules.length, rules },
    });
    ...
    console.error('[api/routing/rules/reorder POST] Error:', err);
    return NextResponse.json(
      { success: false, error: 'Failed to reorder routing rules' },
      { status: 500 },
    );
  }
}
