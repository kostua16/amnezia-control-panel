...
const { findPendingPullRequest } = require('../check-pending-automation-pr.cjs');
...
test('returns the matching PR when title prefix matches and it is not self', () => {
  const prs = [
    { number: 1, title: AUDIT_PREFIX, headRefName: 'claude-a' },
  ];
...
test('excludes self by head ref even when the title matches', () => {
  const prs = [
    { number: 3, title: AUDIT_PREFIX, headRefName: 'self-branch' },
  ];
  const result = findPendingPullRequest(prs, {
    titlePrefix: AUDIT_PREFIX,
...
test('returns null for an empty pull request list', () => {
  assert.equal(
    findPendingPullRequest([], { titlePrefix: AUDIT_PREFIX, excludeHead: 'self' }),
    null,
  );
});

test('skips null and non-object entries without throwing', () => {
  const prs = [null, 'nope', undefined, { number: 5, title: AUDIT_PREFIX, headRefName: 'y' }];
  const result = findPendingPullRequest(prs, {
    titlePrefix: AUDIT_PREFIX,
    excludeHead: 'self',
  });
