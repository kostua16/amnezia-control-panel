...
| # | Proposal | Severity | Area | Status |
|---|----------|----------|------|--------|
| 47 | **Remove shell metacharacter guard from HTTP-bound config-applier values** — `validateNoInjection()` rejects values with `;|&$\`\\` but the push path is purely HTTP (`httpClient` → JSON body). No shell involved. The guard rejects legitimate WireGuard endpoints containing `$` (IPv6 zone IDs), `&` (URLs), or `\` (paths). Injection safety is already provided by `execFile()` array args. Fix: remove `DANGEROUS_CHARS`, `validateNoInjection()`, and three calls at lines 131-136 | Medium (Correctness) | `src/lib/config-applier.ts:8-14, 131-136` | Proposed |
